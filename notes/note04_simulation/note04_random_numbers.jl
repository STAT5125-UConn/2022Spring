...
begin
	using Plots, PlutoUI, StatsBase, Distributions
...
md"""
In fact, you can generate not only random vectors, but also random matrices:
"""

# ╔═╡ 4aa89796-11cd-4194-975b-56b9fa08c5e2
rand(3, 5)
...
md"""
The function `rand` performs **uniform sampling**, i.e. each possible object as the *same* probability. This can also be done with the `sample` function from the `StatsBase.jl` package.
...
We see that `countmap` returns a **dictionary** (`Dict`), which maps **keys** to **values**.
"""
...
Let's switch from heads/tails to true/false as the output, and make that into a function:
"""
...
md"""
The `Distributions` package defines many commonly used distributions such as `Binomial(n,p)`, `Poisson(λ)`, `Chisq(ν)`, `Exponential(θ)`, `Gamma(α,θ)` `Normal(μ,σ)`, `MvNormal(μ, Σ)`, etc.

Note that for Exponential(1), random numbers can also be generated with `randexp` from the `Random` package; for Normal(μ,σ), random numbers can also be generated with `randn`. 
"""
...
# ╔═╡ 5ec8c9ec-8897-4597-b173-425e6b2e7423
begin
    d₁ = 4
    μ₁ = 1:d₁
...
# ╔═╡ f438b1d3-99e0-4213-b48a-e3da7798788f
[0.5^abs(i-j) for i in 1:d₁, j in 1:d₁]

# ╔═╡ 49fe72b2-eb65-49f7-af7a-f7ce6392b256
md"""
...
md"""
The LLN indicates that, in many situation, the average of the results obtained from performing the same experiment a large number of times should be tend to become closer to the expected value.
"""

# ╔═╡ 20bfb207-2fd5-4492-86a2-5ad2a65778d8
md"""
n = $(@bind n Slider(0:10:1000, show_value=true, default=10))
"""
...
md"""
The CLT establishes that, in many situations, normalized average or sum of independent random variables tends toward a normal distribution even if the original variables themselves are not normally distributed. 
"""

# ╔═╡ 582fd476-7b51-42a0-8c49-635a939ed8bf
md"""
Bernoulli: n = $(@bind n_ber Slider(1:1:1000, show_value=true, default=1))
"""
...
md"""
Chi square: n = $(@bind n_chisq Slider(1:1:1000, show_value=true, default=1))
"""
...
md"""
T: n = $(@bind n_t Slider(1:1:1000, show_value=true, default=1))
"""

# ╔═╡ 64849e25-12e2-415a-baac-867239886bbc
begin
    data_t = [mean(rand(TDist(1), n_t)) for _ in 1:1000]
...
md"""
## Connection between normal distribution and Chi square distribition
"""

# ╔═╡ 9a1136c2-873c-11eb-124f-c3939972ce4a
md"""
dof = $(@bind dof Slider(1:500, show_value=true))  
"""
