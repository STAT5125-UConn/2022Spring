...
unique([1, missing, 2, missing])
levels([1, missing, 2, missing])
...
df = disallowmissing(df, error=false) # column :x1 is left untouched as it contains missing
eltype.(eachcol(df))
...
df = DataFrame(a = [1,missing,missing], b=[1,2,3])
...
sum.(eachrow(ismissing.(df)))

df[sum.(eachrow(ismissing.(df))) .< ncol(df),:]
...
ENV["GKSwstype"]="nul"
weave("03_missingvalues.jl", doctype="github")
